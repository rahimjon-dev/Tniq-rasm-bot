...
export const translations = {
  uz: {
    choose_language:
      `🌐 <b>Assalomu alaykum! Iltimos, o'zingizga qulay tilni tanlang:</b>\n\n` +
    ...
    image_received: (w: number, h: number, rem: number, max: number) =>
    ...
    processing_image: (scale: number) =>
      `🤖 <b>AI orqali rasm tiniqlashtirilmoqda (${scale}x)...</b> ⏳\n<i>(Neyron tarmoq piksellarni tiklamoqda, 3-5 soniya kuting)</i>`,
    processing_video: (res: string) =>
      `🎬 <b>Video kadrlari AI orqali tiniqlashtirilmoqda (${res})...</b> ⏳\n<i>(Har bir kadr neyron tarmoqdan o'tkazilmoqda)</i>`,
    ...
    account_info: (id: number | bigint, name: string, username: string, total: number, date: string) =>
    ...
    usage_info: (date: string, imgUsed: number, imgMax: number, imgRem: number, vidUsed: number, vidMax: number, vidRem: number) =>
  ...
  en: {
    choose_language:
      `🌐 <b>Please choose your preferred language:</b>\n\n` +
  ...
  ru: {
    ...
    usage_info: (date: string, imgUsed: number, imgMax: number, imgRem: number, vidUsed: number, vidMax: number, vidRem: number) =>
      `📊 <b>Статистика использования за сегодня (${date})</b>\n\n` +
