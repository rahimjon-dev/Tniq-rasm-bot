...
const userLastMessageTimes = new Map<number, number>();
...
export function acquireUserJobLock(userId: string): boolean {
...
export function releaseUserJobLock(userId: string): void {
...
export async function rateLimitMiddleware(ctx: Context, next: () => Promise<void>): Promise<void> {
  ...
  if (userLastMessageTimes.size > 5000) {
    ...
    for (const [id, time] of userLastMessageTimes.entries()) {
      if (time < cutoff) userLastMessageTimes.delete(id);
    }
  }
