...
export const pendingVideoUploads = new Map<number, {
  userId: string;
  filePath: string;
...
export async function handleIncomingVideo(ctx: Context): Promise<void> {
  ...
  try {
    ...
    await ctx.replyWithHTML(
      t.video_received(metadata.width, metadata.height, metadata.fps, metadata.durationSeconds, quota.remaining, quota.maxLimit),
