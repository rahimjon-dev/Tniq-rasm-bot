...
export async function handleScaleSelection(ctx: Context, scale: 2 | 4): Promise<void> { 
  ...
  await ctx.editMessageText(
    t.queued_image(jobId, scale, pending.originalWidth, pending.originalHeight),
    { parse_mode: 'HTML' }
  );
...
export async function handleVideoResolutionSelection(
  ctx: Context,
  ...
  await ctx.editMessageText(
    t.queued_video(jobId, resolution, scale, pending.durationSeconds, pending.fps),
    { parse_mode: 'HTML' }
  );
