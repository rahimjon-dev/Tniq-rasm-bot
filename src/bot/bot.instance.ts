...
// 🎬 Upscale Video
bot.hears(
  ['🎬 Video 4K qilish', '🎬 Upscale Video', '🎬 Video Tiniqlashtirish', '🎬 Улучшить видео', '/video'],
  async (ctx) => {
    const lang = (await UserService.getUserLanguage(ctx.from?.id)) || 'uz';
    const t = getT(lang);
    await ctx.replyWithHTML(
...
bot.hears(['👤 My Account', '👤 Profilim', '👤 Mening Hisobim', '👤 Мой профиль', '/account'], async (ctx) => {
  ...
  await ctx.replyWithHTML(
    t.account_info(telegramId, ctx.from?.first_name || 'User', ctx.from?.username || '', totalJobs, joinedDate),
    getMainKeyboard(lang)
  );
});
...
bot.hears(['📊 My Usage', '📊 Limitlar', '📊 Kunlik Limitlar', '📊 Лимиты', '/usage'], async (ctx) => {
  ...
  await ctx.replyWithHTML(
    t.usage_info(
      ...
      summary.imagesUsed,
      summary.imagesMax,
      summary.imagesRemaining,
      summary.videosUsed,
      summary.videosMax,
      summary.videosRemaining
    ),
    getMainKeyboard(lang)
  );
});
