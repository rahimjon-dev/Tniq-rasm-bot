...
import {
  ...
  handleVideoResolutionSelection,
  handleCancelAction,
} from './handlers/callback.handler.js';
import {
  handleProCustomBackgroundCommand,
  handleResetCustomBackground,
  awaitingProBackgroundUsers,
} from './handlers/pro-background.handler.js';
import {
  handleStarRatingCallback,
  checkAndProcessReviewComment,
  handleReviewCommand,
  pendingReviewRatings,
...
bot.hears(
  ...
  async (ctx) => {
    const lang = (await UserService.getUserLanguage(ctx.from?.id)) || 'uz';
    const t = getT(lang);
    if (ctx.from?.id) UserService.updateActivity(ctx.from.id, 'Viewed settings');
...
bot.hears(
  ['🌐 Change Language', '🌐 Tilni o\'zgartirish', '🌐 Сменить язык', '/language', '/lang'],
  async (ctx) => {
    const lang = (await UserService.getUserLanguage(ctx.from?.id)) || 'uz';
    const t = getT(lang);
    await ctx.replyWithHTML(t.choose_language, languageKeyboard);
...
bot.hears(
  ...
  async (ctx) => {
    const lang = (await UserService.getUserLanguage(ctx.from?.id)) || 'uz';
    const t = getT(lang);
    if (ctx.from?.id) UserService.updateActivity(ctx.from.id, 'Image mode');
...
bot.hears(
  ...
  async (ctx) => {
    const lang = (await UserService.getUserLanguage(ctx.from?.id)) || 'uz';
    const t = getT(lang);
    if (ctx.from?.id) UserService.updateActivity(ctx.from.id, 'Video mode');
    await ctx.replyWithHTML(
      t.video_mode(config.MAX_VIDEO_SIZE_MB, config.MAX_VIDEO_DURATION_SECONDS),
...
bot.hears(
  ...
  async (ctx) => {
    ...
    const t = getT(lang);
    const totalJobs = (user as any).totalJobs || (await UserService.getUserTotalJobsCount(user.id));
  ...
  async (ctx) => {
    ...
    await ctx.replyWithHTML(
      t.usage_info(
        ...
        planName,
        summary.imagesUsed,
        summary.isUnlimitedImages ? 'Cheksiz / Unlimited' : summary.imagesMax,
        summary.isUnlimitedImages ? 'Cheksiz' : summary.imagesRemaining,
        summary.videosUsed,
        summary.isUnlimitedVideos ? 'Cheksiz / Unlimited' : summary.videosMax,
        summary.isUnlimitedVideos ? 'Cheksiz' : summary.videosRemaining
      ),
      getMainKeyboard(lang)
    );
  }
...
bot.hears(['❓ Yordam', '❓ Help', '❓ Помощь', 'ℹ️ Help', 'ℹ️ Yordam', 'ℹ️ Помощь', '/help'], async (ctx) => {
  const lang = (await UserService.getUserLanguage(ctx.from?.id)) || 'uz';
  const t = getT(lang);
  if (ctx.from?.id) UserService.updateActivity(ctx.from.id, 'Viewed help');
...
bot.action('admin_broadcast_info', async (ctx) => {
  ...
  await ctx.answerCbQuery();
  await ctx.replyWithHTML(
    `📢 <b>Barcha foydalanuvchilarga xabar yuborish uchun:</b>\n\n` +
...
bot.action('admin_web_link', async (ctx) => {
  ...
  await ctx.answerCbQuery();
  await ctx.replyWithHTML(
    `🌐 <b>AI MEDIA UPSCALER — VEB ADMIN DASHBOARD</b>\n\n` +
