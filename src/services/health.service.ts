...
function readJsonBody(req: http.IncomingMessage, maxBytes = 50 * 1024 * 1024): Promise<any> {
      ...
      try {
        ...
        req.on('end', async () => {
          try {
            ...
            setImmediate(() => {
              bot.handleUpdate(update).catch((updateErr: any) => {
      ...
      try {
        ...
        if (type === 'IMAGE') {
          ...
          setImmediate(async () => {
            ...
            try {
              try {
                progressMsg = await bot.telegram.sendMessage(
                  Number(tid),
                  '⚡ <b>AI 4K Tiniqlashtirish boshlandi...</b>\n<i>Natija bir necha soniyada tayyorlanib yuboriladi.</i>',
                  { parse_mode: 'HTML' }
                );
              } catch {}
        ...
        if (type === 'VIDEO') {
          ...
          const fileId = crypto.randomUUID();
          const targetResolution = (body.resolution || '4K') as '720p' | '1080p' | '2K' | '4K';
          ...
          setImmediate(async () => {
            try {
              let scale = 2;
              if (targetResolution === '4K') scale = 4;
