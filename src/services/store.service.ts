...
class StoreService {
  ...
  getAllUsers(query = '', page = 1, limit = 20, planFilter = '', statusFilter = '') {
    ...
    for (const u of list) {
      ...
      if (lastActive >= oneDayAgo) activeUsersCount++;
      if (u.plan === 'PRO' || u.plan === 'PREMIUM' || (u.plan as any) === 'BUSINESS') proUsersCount++;
    }
  ...
  getRecentJobs(limit = 10): StoredJob[] {
    if (!Array.isArray(this.data.jobs)) return [];
