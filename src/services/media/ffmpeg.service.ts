...
export class FFmpegService {
  private static readonly ffmpegExe: string = (ffmpegPath as any)?.default || (ffmpegPath as any);
  private static readonly ffprobeExe: string = (ffprobePath as any)?.path || (ffprobePath as any);
  ...
  static async getMetadata(filePath: string): Promise<VideoMetadata> {
    ...
    const durationSeconds =
      parseFloat(videoStream.duration || parsed.format?.duration || '0') || 0;

    const totalFrames =
      parseInt(videoStream.nb_frames, 10) || Math.round(durationSeconds * fps) || 0;

    const bitrateKbps = Math.round(
      (parseInt(parsed.format?.bit_rate || videoStream.bit_rate || '0', 10)) / 1000
    );
   ...
   */
  static async extractAudio(videoPath: string, audioOutputPath: string): Promise<boolean> {
