...
export const pendingReviewRatings = new Map();
/**
 * Send interactive star rating invitation after image/video processing completes
 */
export async function sendReviewInvitation(telegramChatId, lang = 'uz') {
    try {
        const t = getT(lang);
        await bot.telegram.sendMessage(telegramChatId, t.review_prompt_after_job, {
            parse_mode: 'HTML',
 ...
 */
export async function handleStarRatingCallback(ctx, rating) {
 ...
 */
export async function checkAndProcessReviewComment(ctx) {
    const telegramId = ctx.from?.id;
    if (!telegramId)
        return false;
    ...
    for (const adminId of config.ADMIN_TELEGRAM_IDS) {
        try {
            await bot.telegram.sendMessage(Number(adminId), `🌟 <b>YANGI BAHOLASH VA IZOH!</b>\n\n` +
