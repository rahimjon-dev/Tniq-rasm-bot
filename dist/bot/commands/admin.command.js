...
export async function handleAdminCommand(ctx) {
    ...
    const adminText = `👑 <b>AI MEDIA UPSCALER — ADMINISTRATOR PANELI</b>\n\n` +
...
export async function handleStatsCommand(ctx) {
    ...
    const statsText = `📊 <b>TIZIM VA MONITORING STATISTIKASI</b>\n\n` +
...
export async function handleBroadcastCommand(ctx) {
    ...
    if (!messageToSend) {
        await ctx.replyWithHTML(`📢 <b>Xabar tarqatish formati:</b>\n\n` +
    ...
    await ctx.replyWithHTML(`📢 <b>Xabar tarqatish yakunlandi!</b>\n\n` +
...
export async function handleSetPlanCommand(ctx) {
    const telegramId = ctx.from?.id;
    if (!telegramId || !AdminService.isAdmin(telegramId))
        return;
    // @ts-ignore
...
//# sourceMappingURL=admin.command.js.map