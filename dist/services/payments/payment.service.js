...
export class PaymentService {
    ...
    static async activateFreeBetaPass(telegramId) {
        ...
        return {
            success: true,
            message: `🎉 <b>Tabriklaymiz! Sizga BEPUL BETA PRO tarifi faollashtirildi!</b>\n\n` +
     ...
     */
    static async sendStarsInvoice(telegramId, plan) {
        const user = await UserService.findOrCreateUser({ telegramId });
        if (plan === 'PRO') {
     ...
     */
    static async handleSuccessfulPayment(ctx) {
        // @ts-ignore
        ...
        try {
            ...
            // 3. Congratulate user
            await ctx.replyWithHTML(`🎉 <b>To'lovingiz muvaffaqiyatli qabul qilindi!</b>\n\n` +
