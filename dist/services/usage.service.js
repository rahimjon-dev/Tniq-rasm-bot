...
export class UsageService {
    ...
    static getMaxImagesForPlan(plan) {
        switch (plan) {
            case 'BUSINESS':
    ...
    static async canProcessImage(userId, plan) {
    ...
    static async incrementImageUsage(userId) {
        const today = this.getTodayDateString();
        if (!isDatabaseAvailable()) {
            const key = `${userId}_${today}`;
            const record = inMemoryUsage.get(key) || { images: 0, videos: 0 };
    ...
    static getMaxVideosForPlan(plan) {
        switch (plan) {
            case 'BUSINESS':
                return 50;
            case 'PRO':
    ...
    static async canProcessVideo(userId, plan) {
        const today = this.getTodayDateString();
    ...
    static async incrementVideoUsage(userId) {
        const today = this.getTodayDateString();
        if (!isDatabaseAvailable()) {
            const key = `${userId}_${today}`;
            const record = inMemoryUsage.get(key) || { images: 0, videos: 0 };
    ...
    static async getUserUsageSummary(userId, plan) {
        const today = this.getTodayDateString();
