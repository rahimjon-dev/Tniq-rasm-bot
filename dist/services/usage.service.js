...
export class UsageService {
     ...
     */
    static async getUserUsageSummary(userId, telegramId, plan) {
     ...
     */
    static async canProcessImage(userId, telegramId, plan) {
        const limits = PlanService.getLimits(plan);
     ...
     */
    static async canProcessVideo(userId, telegramId, plan) {
        const limits = PlanService.getLimits(plan);
     ...
     */
    static async reserveImageQuota(userId, telegramId, plan) {
    ...
    static releaseImageReservation(telegramId) {
        const key = telegramId.toString();
        const cur = inFlightReservations.get(key);
        if (cur && cur.images > 0) {
            cur.images -= 1;
            if (cur.images === 0 && cur.videos === 0)
                inFlightReservations.delete(key);
        }
    }
    static async reserveVideoQuota(userId, telegramId, plan) {
    ...
    static releaseVideoReservation(telegramId) {
        const key = telegramId.toString();
        const cur = inFlightReservations.get(key);
        if (cur && cur.videos > 0) {
            cur.videos -= 1;
     ...
     */
    static async incrementImageUsage(userId, telegramId) {
     ...
     */
    static async incrementVideoUsage(userId, telegramId) {
     ...
     */
    static async resetUserUsage(userId, telegramId) {
